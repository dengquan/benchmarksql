...
public class LoadData implements jTPCCConfig {
  ...
  private static int                queryTimeoutSec = 0;
  ...
  public static void main(String[] args) {
    ...
    private static void intPara() {
        ...
        try {
            ...
            queryTimeoutSec = Integer.parseInt(ini.getProperty("queryTimeoutSec","0"));
...
static void initJDBC() {
  ...
  try {
    ...
    distPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".district " +
       " (d_id, d_w_id, d_ytd, d_tax, d_next_o_id, d_name, d_street_1, d_street_2, d_city, d_state, d_zip) " +
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    distPrepStmt.setQueryTimeout(queryTimeoutSec);

    itemPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".item " +
       " (i_id, i_name, i_price, i_data, i_im_id) " +
       "VALUES (?, ?, ?, ?, ?)");
    itemPrepStmt.setQueryTimeout(queryTimeoutSec);

    custPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".customer " +
       " (c_id, c_d_id, c_w_id, " +
       ...
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    custPrepStmt.setQueryTimeout(queryTimeoutSec);

    histPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".history " +
       " (hist_id, h_c_id, h_c_d_id, h_c_w_id, " +
         "h_d_id, h_w_id, " +
         "h_date, h_amount, h_data) " +
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    histPrepStmt.setQueryTimeout(queryTimeoutSec);
    ...
    ordrPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".oorder " +
       ...
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ordrPrepStmt.setQueryTimeout(queryTimeoutSec);

    orlnPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".order_line " +
       " (ol_w_id, ol_d_id, ol_o_id, " +
         "ol_number, ol_i_id, ol_delivery_d, " +
         "ol_amount, ol_supply_w_id, ol_quantity, ol_dist_info) " +
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    orlnPrepStmt.setQueryTimeout(queryTimeoutSec);

    nworPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".new_order " +
       " (no_w_id, no_d_id, no_o_id) " +
       "VALUES (?, ?, ?)");
    nworPrepStmt.setQueryTimeout(queryTimeoutSec);

    stckPrepStmt = conn.prepareStatement
      ("INSERT INTO " + keyspace + ".stock " +
       " (s_i_id, s_w_id, s_quantity, s_ytd, s_order_cnt, s_remote_cnt, s_data, " +
         "s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, " +
         "s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10) " +
       "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    stckPrepStmt.setQueryTimeout(queryTimeoutSec);

    whsePrepStmt = conn.prepareStatement
       ("INSERT INTO " + keyspace + ".warehouse " +
        " (w_id, w_ytd, w_tax, w_name, w_street_1, w_street_2, w_city, w_state, w_zip) " +
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    whsePrepStmt.setQueryTimeout(queryTimeoutSec);
